...
class ISICDataset(Dataset):
    def __init__(self, root_dir, transform=None):
        ...
        """
        # Get image names, filtering for common extensions
    ...
    def __getitem__(self, idx):
        ...
        # Load image and ensure it is RGB
...
def load_ISIC(config, loadtest=False, ntest=2048, index=0):
    '''
    Loads ISIC dataset keeping 3 RGB channels.
    '''
    # Initial transform without Grayscale
    ...
    isic_dataset = ISICDataset(config.path_data, transform)
    
    # Load images between index and index+1 times the number of data
    start_idx = index * config.n_images
    end_idx = min((index + 1) * config.n_images, len(isic_dataset))
    indices = np.arange(start_idx, end_idx) 
    trainset = torch.utils.data.Subset(isic_dataset, indices)
    testset = None
    ...
    if config.CENTER:
        # Load a subset to compute mean/std for normalization
        ...
        # New transform with normalization
        transform = transforms.Compose([
             transforms.ToTensor(),
             transforms.Resize(config.IMG_SHAPE[1]),
             transforms.CenterCrop(config.IMG_SHAPE[1]),
