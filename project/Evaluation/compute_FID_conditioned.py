...
def detransform_images(images, config):
    ...
    std = torch.tensor(config.std, dtype=images.dtype,
                       device=images.device).view(1, -1, 1, 1)
    return t * std + mean
...
def compute_fid_for_checkpoint(tau, type_model, config, path_stats_testset, 
    ...
    try:
        ...
        args = '{:s} {:s} --device {:s}'.format(path_stats_testset,
                                                     file_img_gen,
                                                     config.DEVICE)
...
def main():
    ...
    config.IMG_SHAPE = (3, args.img_size, args.img_size)  # Changed 1 to 3
