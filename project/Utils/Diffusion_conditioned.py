...
def sample_diffusion_from_noise(model, n_images=25, config=TrainingConfig(), 
                                df=DiffusionConfig(), dim=3, c=None):
    ...
    for t in reversed(range(0, config.TIMESTEPS)):
        ...
        # Process the class label 'c'
        if c is None:
            # Generate unconditional images (class 0)
            c_input = torch.zeros(n_images, dtype=torch.long, device=config.DEVICE)
        else:
            c_input = c
            
        # Predict the noise at times ts
        eps_ts = model(x, ts, c_input)
...
def sample_diffusion_from_noise_DDIM(model, n_images=25, config=TrainingConfig(), 
                                df=DiffusionConfig(), dim=3, eta=0.0, ddim_steps=None, c=None):
    """
    ...
    for i in range(len(time_steps) - 1):
        ...
        # Create an unconditional label (zeros)
        c_uncond = torch.zeros_like(c_input)

        # Forward pass 1: Unconditional prediction
        eps_uncond = model(x, ts, c_uncond)

        # Forward pass 2: Conditional prediction
        eps_cond = model(x, ts, c_input)

        # CFG Extrapolation: push the conditional prediction away from the unconditional
        # guidance_scale = 1.0 means no guidance. guidance_scale > 1.0 strengthens the condition.
        guidance_scale = 2.0 
        eps_ts = eps_uncond + guidance_scale * (eps_cond - eps_uncond)
...
# Training functions
#==========================================
# Add 'c' as a parameter
def train_one_batch(X, c, model, optimizer, loss_fn, 
    ...
    # NEW: Classifier-Free Guidance (CFG) Dropout
    # Randomly drop the class label 10% of the time (set it to 0)
    drop_prob = 0.1
    drop_mask = torch.rand(c.shape, device=config.DEVICE) < drop_prob
    ...
    # Apply the model - NOW passing the class label 'c'
    Y = model(X_t.float(), ts, c)
...
def train(model, trainloader, optimizer, config, df, loss_fn,
    ...
    while n_steps < config.N_STEPS:
        # NEW: Unpack X and c from the dataloader
            ...
            if shallSave == 1:
                ...
                if generate:
                    if len(X.shape) == 4: # For images, assumes [B, C, H, W]
                        ...
                        # Create a diverse dummy class vector to test the conditioning
                        # This array contains 16 items: Two unconditional (0), 
                        # and a mix of Types 1 through 6.
                        ...
                        # Generate the 16 samples using the dummy classes
                        # Note: We use sample_diffusion_from_noise (the training sampler)
                        samples, samples_init = sample_diffusion_from_noise(
                            model, 
                            n_images=16, 
                            config=config, 
                            df=df, 
                            dim=4, 
                            c=dummy_c  # Pass the classes here!
                        )
                        
                        # Plot the images and reverse the normalization
                        fig = Plot.imshow(samples.cpu(), config.mean, config.std)
                        
                        # Save the PDF to the Images/ folder
                        sample_path = config.path_save + suffix + 'Images/' + 'Sample_{:d}.pdf'.format(n_steps)
                        fig.savefig(sample_path, bbox_inches='tight')
                        plt.close('all')
            
            # Pass 'c' into train_one_batch
            loss, _, = train_one_batch(X, c, model, optimizer, loss_fn, config, df)
            n_steps += 1
