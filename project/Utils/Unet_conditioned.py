...
class UNet(nn.Module):
    def __init__(
        ...
        dropout_rate=0.1,
        num_classes=7,          # NEW: Number of classes (0=null, 1-6=Fitzpatrick)
    ):
        super().__init__()
        ...
        if num_classes is not None and num_classes > 0:
            # The class embedding is sized to match the time embedding
            self.class_embeddings = nn.Embedding(num_classes, time_emb_dims_exp)
        else:
    ...
    # NEW: Added `c=None` to the arguments
    def forward(self, x, t, c=None):
        ...
        # NEW: Process class embedding and add it to time embedding
        if self.class_embeddings is not None:
            if c is None:
                # If no class is passed, default to 0 (the unconditional/null class)
                c = torch.zeros_like(t)
            else:
                c = c.flatten().to(torch.long)
                
            class_emb = self.class_embeddings(c)
